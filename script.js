...
$(function () {
  ...
  saveButtonEl.on("click", function (event) {
    localStorage.setItem(
      "hour" + event.target.closest("div").id,
      event.target.closest("div").id
    );

    var actualEventtext = $($(this).prev()[0]).val();
    localStorage.setItem(
      "event" + event.target.closest("div").id,
      actualEventtext
    );
  });
});
...
hourElement.each(function () {
  var diffFromCurrentHour = dayjs(parseInt(this.id.replace("hour-", ""))).diff(
    currentHour
  );
...
function getLocalStorageValues() {
  hourElement.each(function () {
    ...
    if (hourElementID === hourID) {
      ...
      $(this)
        .find(".description")
        .text(localStorage.getItem("event" + $(this).attr("id")));
    }
  });
}
